...
#            print("order submitted")
        ...
        if open_orders_count <= 1:
            ...
            last_prices_bars = self.get_historical_prices(BTC_ASSET, 6, timestep='minute')
            if last_prices_bars and len(last_prices_bars.df) > 2:
                df_last_prices = last_prices_bars.df
                mean_price = df_last_prices[:-1]['high'].mean()
                current_price = self.get_last_price(BTC_ASSET, USDT_ASSET)
                ...
                if mean_price < current_price:
                    ...
                    self.submit_order(order)
                    #cash reward
                    order_reward = self.create_order(asset=BTC_ASSET, quantity=quantity, side="sell", limit_price=current_price + current_price*0.05)
