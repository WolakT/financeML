...
def import_cot_data(start_year, end_year, market):
    ...
    for i in range(start_year, end_year + 1):
        single_year = pd.DataFrame(
            cot.cot_year(i, cot_report_type="traders_in_financial_futures_fut")
        )
    ...
    new_df["Report_Date_as_YYYY-MM-DD"] = pd.to_datetime(
    ...
    data["Net_COT"] = (
        data["Pct_of_OI_Lev_Money_Long_All"] - data["Pct_of_OI_Lev_Money_Short_All"]
    ) - (data["Pct_of_OI_Dealer_Long_All"] - data["Pct_of_OI_Dealer_Short_All"])
...
def plot_train_test_values(window, train_window, y_train, y_test, y_predicted):
    ...
    plt.plot(
        plotting_time_series[:, 0], label="Training data", color="black", linewidth=2.5
    )
    plt.plot(
        plotting_time_series[:, 1],
        label="Test data",
        color="black",
        linestyle="dashed",
        linewidth=2,
    ...
    plt.plot(
        plotting_time_series[:, 2], label="Predicted data", color="red", linewidth=1
    )
...
def import_crypto(symbol, start_date, end_date, interval="1h", limit=500):
    # Getting the original link from Binance
    url = "https://api.binance.com/api/v1/klines"
    # Linking the link with the Cryptocurrency and the time frame
    link = url + "?symbol=" + symbol + "&interval=" + interval + "&limit=" + str(limit) + "&startTime=" + str(start_date) + "&endTime=" + str(end_date)
...
def validate_datetime_index(df):
    # Check if the index is a DatetimeIndex
    if not isinstance(df.index, pd.DatetimeIndex):
        return False, "Index is not a DatetimeIndex."

    # Calculate the time difference between each consecutive index
    time_diff = df.index.to_series().diff().dropna()

    # Check if all time differences are exactly one minute
    if not all(time_diff == pd.Timedelta(minutes=1)):
        return False, "Gaps found in the datetime index."

    return True, "No gaps in the datetime index."
